...
def calculte_distance_view(request):
    ...
    # location marker
    folium.Marker([l_lat, l_lon], tooltip = 'click here more',popup = city['city'],icon = folium.Icon(color = 'purple')).add_to(m)
    ...
    if form.is_valid():
        ...
        folium.Marker([l_lat, l_lon], tooltip = 'click here more',popup = city['city'],icon = folium.Icon(color = 'purple')).add_to(m)
        #destination marker
        folium.Marker([d_lat, d_lon], tooltip = 'click here more', popup = destination, icon = folium.Icon(color = 'blue', icon = 'cloud')).add_to(m)
